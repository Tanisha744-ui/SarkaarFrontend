...
export class ImposterGame implements OnDestroy, OnInit {
  ...
  // Step 2: Create game with player names only
  // createGame() {
  ...
  // // Step 3: Register each player
  // registerPlayer() {
  //   this.gameService.registerPlayer(this.gameId, this.name).subscribe(res => {
  ...
  // Remove old joinGame, use registerPlayer instead
...
startGame() {
  ...
  // submitClue() {
  //   this.gameService.submitClue(this.gameId, this.playerId, this.myClue).subscribe(() => {
  //     this.myClue = '';
  //     this.refreshPlayersAndClues();
  //   });
  // }
//   submitClue() {
//   this.gameService.submitClue(this.gameId, this.playerId, this.myClue)
//     .subscribe(() => {
...
//         // next player turn
//         this.playerId = this.players[this.currentPlayerIndex].playerId;
//         this.getWordAndRole();
//       }
...
// Switch device to current clue-turn player
switchToCurrentTurnPlayer() {
  const p = this.players[this.clueTurnIndex];
  if (!p) return;

  // VERY IMPORTANT
  this.playerId = p.playerId;

  // MUST re-fetch word for THIS player
  this.getWordAndRole();
}
